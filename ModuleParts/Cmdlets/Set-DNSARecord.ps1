<#
.Synopsis
	Set-DNSARecord modifies properties of an existing DNS A Record in the Infoblox database.
.DESCRIPTION
	Set-DNSARecord modifies properties of an existing DNS A Record in the Infoblox database.  Valid IB_DNSARecord objects can be passed through the pipeline for modification.  A valid reference string can also be specified.  On a successful edit no value is returned unless the -Passthru switch is used.
...
.EXAMPLE
	Get-DNSARecord -Gridmaster $Gridmaster -Credential $Credential -Name testrecord.domain.com | Set-DNSARecord -IPAddress 192.168.1.2 -comment 'new comment' -passthru
...
.EXAMPLE
	Set-DNSARecord -Gridmaster $Gridmaster -Credential $Credential -_ref record:a/ZG5zLmJpbmRfYSQuX2RlZmF1bHQuY29tLmVwcm9kLHBkZGNlcGQwMWhvdW1yaWIsMTAuNzUuMTA4LjE4MA:testrecord2.domain.com/default -ClearTTL -Passthru
...
Function Set-DNSARecord{
    ...
    PROCESS{
            ...
            If ($Record){
                $Record | Set-DNSARecord -IPAddress $IPAddress -Comment $Comment -TTL $TTL -ClearTTL:$ClearTTL -Passthru:$Passthru
            }
