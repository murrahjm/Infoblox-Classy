<#
.Synopsis
	Remove-DNSCNameRecord removes the specified DNS CName record from the Infoblox database.
.DESCRIPTION
	Remove-DNSCNameRecord removes the specified DNS CName record from the Infoblox database.  If deletion is successful the reference string of the deleted record is returned.
...
.PARAMETER Record
	An object of type IB_DNSARecord representing the DNS record.  This parameter is typically for passing an object in from the pipeline, likely from Get-DNSCNameRecord.
.EXAMPLE
	Remove-DNSCNameRecord -Gridmaster $Gridmaster -Credential $Credential -_Ref record:cname/ZG5zLmJpbmRfYSQuX2RlZmF1bHQuY29tLmVwcm9kLHBkYWR1dGwwMWNvcnAsMTAuOTYuMTA1LjE5MQ:testalias.domain.com/default
...
.EXAMPLE
	Get-DNSARecord -Gridmaster $Gridmaster -Credential $Credential -name testalias.domain.com | Remove-DNSCNameRecord
