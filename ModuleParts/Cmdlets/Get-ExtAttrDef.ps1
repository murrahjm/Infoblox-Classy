...
.EXAMPLE
	Get-InfobloxView -Gridmaster $Gridmaster -Credential $Credential Type DNSView -IsDefault $True

	This example retrieves the dns view specified as default.
.EXAMPLE
	Get-InfobloxView -Gridmaster $Gridmaster -Credential $Credential -Type NetworkView -comment 'default'

	This example retrieves any network views with the word 'default' in the comment
.EXAMPLE
	Get-InfobloxView -Gridmaster $Gridmaster -Credential $Credential -_Ref 'networkview/ZGdzLm5ldHdvamtfdmlldyQw:Default/true'
    ...
    write-verbose "$FunctionName`:  Beginning Function"
		Try {
			If ($pscmdlet.ParameterSetName -eq 'byRef'){
				Get-InfobloxRecord -Gridmaster $Gridmaster -Credential $Credential -_ref $_Ref
			} else {
				Write-Verbose "$Functionname`:  calling IB_ExtAttrsDef Get method with the following parameters`:"
				Write-Verbose "$FunctionName`:  $gridmaster,$credential,$name,$isDefault,$Comment,$Strict,$MaxResults"
				[IB_ExtAttrsDef]::Get($Gridmaster,$Credential,$Name,$Type,$Comment,$DefaultValue,$Strict,$MaxResults)
