<#
.Synopsis
	Set-DNSPTRRecord modifies properties of an existing DNS PTR Record in the Infoblox database.
.DESCRIPTION
	Set-DNSPTRRecord modifies properties of an existing DNS PTR Record in the Infoblox database.  Valid IB_DNSPTRRecord objects can be passed through the pipeline for modification.  A valid reference string can also be specified.  On a successful edit no value is returned unless the -Passthru switch is used.
...
.EXAMPLE
	Get-DNSPTRRecord -Gridmaster $Gridmaster -Credential $Credential -PTRDName testrecord.domain.com | Set-DNSPTRRecord -PTRDName testrecord2.domain.com -comment 'new comment' -passthru
...
.EXAMPLE
	Set-DNSPTRRecord -Gridmaster $Gridmaster -Credential $Credential -_ref record:ptr/ZG5zLmJpbmRfYSQuX2RlZmF1bHQuY29tLmVwcm9kLHBkZGNlcGQwMWhvdW1yaWIsMTAuNzUuMTA4LjE4MA:2.1.168.192.in-addr.arpa/default -ClearTTL -Passthru
...
Function Set-DNSPTRRecord{
    ...
    PROCESS{
            ...
            If ($Record){
                $Record | Set-DNSPTRRecord -PTRDName $PTRDName -Comment $Comment -TTL $TTL -ClearTTL:$ClearTTL -Passthru:$Passthru
            }
