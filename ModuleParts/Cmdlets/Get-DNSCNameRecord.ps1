<#
.Synopsis
	Get-DNSCNameRecord retreives objects of type DNSCNameRecord from the Infoblox database.
.DESCRIPTION
	Get-DNSCNameRecord retreives objects of type DNSCNameRecord from the Infoblox database.  Parameters allow searching by Name, Canonical, View, Zone or Comment  Also allows retrieving a specific record by reference string.  Returned object is of class type DNSCNameRecord.
...
.EXAMPLE
	Get-DNSCNameRecord -Gridmaster $Gridmaster -Credential $Credential -Canonical 'testrecord.domain.com'

	This example retrieves all DNS records with Canonical of testrecord.domain.com
.EXAMPLE
	Get-DNSCNameRecord -Gridmaster $Gridmaster -Credential $Credential -comment 'Test Comment' -Strict

	This example retrieves all DNS records with the exact comment 'test comment'
.EXAMPLE
	Get-DNSCNameRecord -Gridmaster $Gridmaster -Credential $Credential -_Ref 'record:cname/2ifnkqoOKFNOFkldfjqfko3fjksdfjld:testalias.domain.com/default'

	This example retrieves the single DNS record with the assigned reference string
.EXAMPLE
	Get-DNSCNameRecord -Gridmaster $Gridmaster -Credential $Credential -name testalias.domain.com | Remove-DNSCNameRecord
...
.EXAMPLE
	Get-DNSCNameRecord -Gridmaster $Gridmaster -Credential $Credential -Canonical 'oldserver.domain.com' -Strict | Set-DNSCNameRecord -canonical 'newserver.fqdn.com'
...
.EXAMPLE
	Get-DNSCNameRecord -Gridmaster $Gridmaster -Credential $Credential -Canonical 'oldserver.domain.com' -Strict | Remove-DNSCNameRecord

	This example retrieves all dns cname records pointing to an old server, and deletes them.
.EXAMPLE
	Get-DNSCNameRecord -gridmaster $gridmaster -credential $credential -ExtAttributeQuery {Site -eq 'OldSite'}
